...
class ArraySpec extends Spec {
  ...
  explain(path, via, in_, x) {
    if (!Array.isArray(x)) {
      return [{path, via, pred: (Array.isArray).toString(), val: x, in: in_}];
    }

    if (this.count && this.count !== x.length) {
      const count = this.count;
      return [{path, via, val:x, pred: ((v) => count === v.length).toString(), in: in_}];
    }
    ...
    if ((this.minCount || this.maxCount) &&
      ...
      return [{path, via, val:x,
               pred: ((v) => minCount < v.length && v.length < maxCount).toString(),
               'in': in_}];
    }

    if (this.distinct & x.length > 0 && (new Set(x)).size !== x.length) {
      return [{path, via, val: x,
               pred: (v => (new Set(v)).size === v.length).toString(),
               'in': in_}];
    }
...
class ObjectSpec extends Spec {
  ...
  explain(path, via, in_, x) {
    if (typeof x !== 'object') {
      return [{path, via, pred: ((x) => x === 'object').toString(), val: x, in: in_}];
    }
    const explains = [];
    for (const k in this.required) {
