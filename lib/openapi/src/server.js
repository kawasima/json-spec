...
const jsonSpecs = require(argv.jsonspec);
...
function callback({ method, path, specs}) {
  app[method].call(app, toExpressPath(path), (req, res) => {
    ...
    if (!status && !specs.response[status][accept]) {
      res.sendStatus(500);
    } else {
...
(async () => {
  ...
  app.listen(argv.port, () => console.log('started'));
})();
